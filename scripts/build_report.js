...
// v11.60(B):优先信任服务端显式标记 meta.lateCapture —— 旧实现只靠 generatedAt 推断,
//   而"18:49 采的 midday"与"15:30 采的 midday"在旧实现里都被归为"收盘后补采",无法区分严重程度。
...
function lateCaptureNote(m) {
  ...
  if (m && m.lateCapture) {
    ...
    return '<div class="hero-warn hero-warn-late">⛔ 补采数据 —— 本页标称 <b>' + esc(s.slot) +
      '</b>,实际采集于 <b>' + genHM + '</b>,内容是该时刻的实时快照,<b>不是 ' + esc(ideal) +
      ' 的盘面</b>。请勿据此判断当时盘中信号。</div>';
  }
...
function renderCloseEmotion(report) {
  ...
  // v11.62:盘中槽位已停用。此块保留给 midday(历史页/补跑),盘前页由 renderPremarketReport 单独渲染
  //   （盘前不走 renderCloseEmotion，见 renderReport 里的分发）。
  const _btData = (report.limitUp || []).length > 0 && (report.mainRank || []).length > 0;
  const _btSlot = (m.type === 'midday') && _btData;
  // v11.62: 区块已抽为 renderBoardTierBlock()，见文件末尾。
  let boardTierMidday = '';
  if (_btSlot) boardTierMidday = renderBoardTierBlock(report, { isPre: m.type === 'premarket' });
  ...
  // v11.62:只有 midday(已停用)才隐藏;盘前保留 —— 盘前正需要"明日观察锚点"与"今天怎么看"。
  ...
  return '<div class="close-emotion">' +
    ...
    '<div class="ce-title">' + esc(m.date || '') + ' ' + esc(m.typeLabel || '') + (m.type === 'midday' ? ' · 实时更新中' : (m.type === 'premarket' ? ' · 盘前基准' : ' · 数据截至 ' + esc(m.time || '15:00'))) + '</div>' +
    topPanel +
    emotion +
    broadBlock +
    flowBlock +
    (boardTierMidday || ladderBlock) +    anchorBlock +
    todayWatchBlock +
    banner +
  '</div>';
}
...
function renderSignalRadar(report) {
  ...
  const rows = list.map(x => {
    ...
    if (x.volRatio != null) meta.push('量比 ' + x.volRatio);
...
build();
/* v11.62: 04 区块(涨停梯队+主线研判+关注锚点+板块强弱+资金切换)抽为独立函数，
   供盘前页(renderPremarketReport)与历史 mid 页共用。
   opts.isPre = true 时文案改为"昨日收盘基准"(盘前语义)。 */
function renderBoardTierBlock(report, opts) {
  opts = opts || {};
  const ce = (report && report.closeEmotion) || {};
  const m = (report && report.meta) || {};
  let boardTierMidday = '';
  ...
  // 结构辨证(动态)
  // v11.62b 纠正:此前按"盘前=上一交易日基准"分叉措辞,v11.62c 经代码取证推翻 ——
  //   `cloud_fetch.mjs` 的 `fetchZT(todayCompact)` 传的是**今天**,盘前简报的涨停池/板块
  ...
  //   仅**收尾提示**按槽位分叉:盘前页是"开盘前后"视角,历史 mid 页是"午后/盘后"视角)
  const _isPre = m.type === 'premarket';
  const _btBanner = '<div class="bt-banner"><span class="bt-b-dot"></span><b>实时追踪 · 主力资金 · 连板梯队</b><span class="bt-b-tip">' +
    (_isPre ? '今日主线预判 · 资金切换方向 · 风险提示' : '盘后接力判断 · 主线确认 · 风险提示') + '</span></div>';
  const _btHint = _isPre
    ? '<div class="hint">今日核心:资金切换信号 + 板块强弱 + 主线研判 三维判断(早盘快照);开盘后紧盯龙头表态。</div>'
