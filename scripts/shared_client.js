...
// v11.0:强制重置本地缓存(清空所有 ATDS 本地状态,强制从云端重新拉取);用户自救急救按钮
function resetLocalCache(){
  var KEYS=['atds_watchlist','atds_watchlist_data','atds_watchlist_hidden','atds_kline_cache','atds_ff_cache','atds_events_cache','atds_trade_status','atds_shadow','atds_qr_code'];
  ...
  var removed=[];
  KEYS.forEach(function(k){try{localStorage.removeItem(k);removed.push(k);}catch(e){}});
  simKeys.forEach(function(k){try{localStorage.removeItem(k);removed.push(k);}catch(e){}});
  console.log('[resetLocalCache] 已清空 keys:', removed.join(', '));
  if(typeof showToast==='function'){showToast('已重置本地缓存,刷新页面强制从云端重新拉取','ok');}
  setTimeout(function(){try{location.reload();}catch(e){}},800);
}
// 二次确认弹窗:防止误点导致本地数据丢失
function confirmResetCache(){
  var msg='⚠️ 确定要重置本地缓存吗?\n\n将清空:\n• 自选股列表(手动添加的股)\n• 隐藏标记(被标记删除的股)\n• 资金/事件/K线缓存\n• 交易状态/模拟跟踪\n• 二维码\n\n云端配置和 review JSON 不受影响。\n刷新页面后将强制从云端重新拉取所有数据。';
