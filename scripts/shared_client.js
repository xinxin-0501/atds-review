...
function buildDecisionCardHtml(s){
  ...
  // 关键位过滤:去重 + 仅保留距现价 ±20% 内(渲染层兜底,剔除远距离整数关口/前高前低噪音)
  function filterLvls(arr,asc){var seen={},a=Array.isArray(arr)?arr:[],out=[];for(var i=0;i<a.length;i++){var x=a[i];if(!x||x.price==null||isNaN(x.price))continue;var k=x.price.toFixed(2);if(seen[k])continue;seen[k]=1;if(price>0&&Math.abs(x.price-price)/price>0.20)continue;out.push(x);}out.sort(asc?function(a,b){return a.price-b.price;}:function(a,b){return b.price-a.price;});return out;}
  function supStrong(){var a=filterLvls(t.supports,false);for(var i=0;i<a.length;i++){if(a[i].weight==='strong')return a[i];}return a[0]||null;}
  function preStrong(){var a=filterLvls(t.pressures,true);for(var i=0;i<a.length;i++){if(a[i].weight==='strong')return a[i];}return a[0]||null;}
  ...
  var supList=filterLvls(t.supports,false);
  var preList=filterLvls(t.pressures,true);
