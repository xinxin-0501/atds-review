...
async function addFetchedToWatchlist(code){
  ...
  // v11.1:自动生成 strategy 字段(logic/capital/plan/strategy/todayStrategy)——手动加股不再缺"逻辑与催化"模块
  try {
    var atds = 70+Math.min(25,Math.max(-15,Math.round(Number(data.pct||0)*2+(Number(data.turnover)||0)*0.5)));
    ...
    data.atds = atds;
  } catch (e) { console.warn('[addFetchedToWatchlist]', code, 'autoGenStrategy 失败:', e.message); }
...
function restoreSavedWatchlist(){
  ...
  (async function(){
    for(var i=0;i<extraCodes.length;i++){
      ...
      // v11.1:补 autoGenStrategy 生成 logic/capital/plan/strategy/todayStrategy,手机端不再缺"逻辑与催化"模块
      try{
        ...
        if(!data.category)data.category='题材';
        data.atds=atdsR;
      }catch(e){console.warn('[restoreSavedWatchlist]', rawCode, 'autoGenStrategy 失败:',e.message);}
