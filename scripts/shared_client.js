...
function buildDecisionCardHtml(s){
  ...
  function minLabel(m){if(!m)return '--';var dir=m.trend==='up'?'多头':m.trend==='down'?'空头':'震荡';var ma=m.ma10!=null?('MA10 '+m.ma10):(m.ma5!=null?('MA5 '+m.ma5):'');var fb=m.fallback?'·日线兜底':'';return ma?(dir+'('+ma+')'+fb):(dir+fb);}
  ...
  var nextDayFocus=(t.ma5&&price)?(price<t.ma5?('明日观察能否放量收复MA5（'+f2(t.ma5)+'），若不能，继续观望。'):('明日观察能否站稳MA5（'+f2(t.ma5)+'）并放量上攻，若失守则减仓。')):'明日观察量能与MA5得失，方向未明前继续观望。';
