...
function buildDecisionCardHtml(s){
  ...
  return '<div class="dc-head">'+
    ...
    '<div class="dc-block dc-review" data-review-code="'+escHtmlF(s.code)+'" data-name="'+escHtmlF(s.name)+'" data-price="'+price+'" data-high="'+(Number(s.high)||price)+'" data-low="'+(Number(s.low)||price)+'" data-entry="'+p.entry+'" data-stop="'+p.stop+'" data-target="'+p.target+'"><div class="dc-h">📊 盘后复盘（当日验证）</div>'+
      ...
      '<div class="dc-line">策略执行/归因：<span class="ts-attr">尚未跟踪 —— 勾选"系统模拟跟踪"后开始累计样本</span> <span class="ts-hint" title="两阶段结算：① 需当日最低回落至计划入场价才算入场（未触发入场不计入样本）② 入场次日起用当日最高/最低判定止盈/止损并自动结算，同日双触保守计为止损。每笔的入场/结算日期与价格见顶部「每笔明细」；有效样本≥10 笔才展示胜率">?</span> <span class="ts-progress-badge">进度 <b class="ts-progress">0/10</b></span> <span class="ts-winrate"></span></div>'+
...
function renderJournalStats(){
  ...
  if(jrnOpen){
    ...
    for(var i=j.length-1;i>=0;i--){
      ...
      var rn={win:'止盈',loss:'止损',expired:'过期按现价',missed:'未触发入场'}[e.result]||(e.result||'—');
      var flow;
      if(legacy){
      ...
      rows+='<div class="jrn-row'+(e.pnl>0?' jrn-pos':(e.pnl<0?' jrn-neg':''))+'">'+
        ...
        '<span class="jrn-num">'+(legacy?'不计入':pnlTxt)+'</span></div>';
    }
    rows+='</div>';
  }
...
function reviewClassification(){
  var j=getJournal();
  var s={valid:0,win:0,loss:0,expired:0,missed:0,legacy:0,sum:0,avg:0,winRate:null};
  ...
  for(var b=0;b<blocks.length;b++){
    ...
    if(r&&r.enabled&&!r.settled){
      ...
      if(!r.entryHit){
        ...
        txt='跟踪中（'+r.entryDate+' 入场@'+r.entry+'）：现价'+px+'，距止盈'+(gap!=null?((gap>=0?'+':'')+gap+'%'):'--')+'（止损'+r.stop+'），触达即自动结算';
      }
    } else {
      var mine=[];
      ...
      if(last&&last.shadow){
        if(last.v!==28){
          ...
          var rn={win:'止盈',loss:'止损',expired:'过期按现价'}[last.result]||(last.result||'已结算');
