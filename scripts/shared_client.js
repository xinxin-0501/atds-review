  ...
  if(!kUsed){
    ...
    if(!rec.entryHit){
      if(low<=entry){
        ...
        if(low<=stop){ doSettle('loss',pct(stop),stop,'入场当日即跌破止损'); return; }
      }
    } else if(rec.entryDate!==today){
      var hitT=high>=target,hitS=low<=stop;
      if(hitT&&hitS){ doSettle('loss',pct(stop),stop,'当日最高触止盈、最低触止损，保守计为止损'); return; }
      if(hitT){ doSettle('win',pct(target),target,'入场后当日最高触及止盈'); return; }
      if(hitS){ doSettle('loss',pct(stop),stop,'当日最低触及止损'); return; }
    }
  }
  // ③ 超期出口
  if(!rec.entryHit){
