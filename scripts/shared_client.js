...
function renderJournalStats(){
  ...
  var warn='<div class="jrn-warn">'+(ex.length?('⚠ '+ex.join('　·　')+'　'):'')+'<button class="jrn-btn jrn-del" onclick="confirmClearReviewData()">清空复盘数据</button></div>';
  var rows='';
  if(jrnOpen){
    ...
    rows+='<div class="jrn-disc">口径说明：用<b>当日最高/最低</b>判定（日线，无法还原日内先后顺序，同日双触保守计为止损）；按<b>计划价</b>成交、<b>不含</b>滑点与手续费；未打开页面的交易日<b>不回溯</b>；未触发入场与计划失效的样本<b>不计入</b>胜率。<b>同一时段多只个股相关性高，胜率样本的有效性应打折看待</b>；样本不足 10 笔仅供参考。</div>';
  }
...
function toggleShadowTrack(cb){
  ...
  var m=getShadowMap();
  if(cb.checked&&block){
    m[code]={enabled:true,name:block.getAttribute('data-name')||code,entry:r2fF(block.getAttribute('data-entry')),stop:r2fF(block.getAttribute('data-stop')),target:r2fF(block.getAttribute('data-target')),openedDate:bjTodayF(),settled:false};
  ...
  var doSettle=function(result,pnl,settlePrice,note,settleDate){
    ...
    if(!dup){
      addJournalEntry({code:code,name:rec.name||code,date:sd,entry:entry,stop:stop,target:target,close:price,pnl:pnl,shadow:true,result:result,note:note||'',
        openedDate:rec.openedDate||'',entryDate:rec.entryDate||'',settleDate:sd,settlePrice:settlePrice,v:28,planKey:planKey,strategy:'swing'});
    }
    setShadowMap(sm);
  };
...
function reviewClassification(){
  var j=getJournal();
  var s={valid:0,win:0,loss:0,expired:0,missed:0,invalid:0,legacy:0,sum:0,avg:0,winRate:null};
