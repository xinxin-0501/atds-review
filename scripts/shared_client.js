  ...
  for(var j=0;j<seq.length;j++){
    ...
    if(!entered){
      if(s.lo<=entry){
        ...
        if(s.lo<=stop){ out={result:'loss',date:s.d,price:stop,note:'入场当日即跌破止损（日K回补）'}; break; }
      }
      continue;                                   // 入场当日不判止盈
    }
    if(s.d===entryDate)continue;
    var hitT=s.hi>=target, hitS=s.lo<=stop;
    if(hitT&&hitS){ out={result:'loss',date:s.d,price:stop,note:'当日最高触止盈、最低触止损，保守计为止损（日K回补）'}; break; }
    if(hitT){ out={result:'win',date:s.d,price:target,note:'日K回补：'+s.d+' 最高'+s.hi+' 触及止盈'}; break; }
    if(hitS){ out={result:'loss',date:s.d,price:stop,note:'日K回补：'+s.d+' 最低'+s.lo+' 触及止损'}; break; }
  }
  return {entered:entered,entryDate:entryDate,settle:out,lastDate:seq[seq.length-1].d};
}
...
// v11.28:归因文本 + 统计口径统一重写(用户反馈"进度7/10、胜率100%"不可信)。
  ...
  for(var b=0;b<blocks.length;b++){
    ...
    if(r&&r.enabled&&!r.settled){
      ...
      if(!r.entryHit){
        ...
        txt='待入场（已观察'+days+'天）：现价'+px+'，需回落至计划入场价'+r.entry+(need!=null?('（还差'+need+'%）'):'')+'；未触发入场前不计入样本';
      }else{
        ...
        txt='跟踪中（'+r.entryDate+' 入场@'+r.entry+'）：现价'+px+'，浮动'+(fl!=null?((fl>=0?'+':'')+fl+'%'):'--')+'，距止盈'+(gap!=null?((gap>=0?'+':'')+gap+'%'):'--')+'（止损'+r.stop+'），触达即自动结算';
      }
    } else {
      var mine=[];
      ...
      if(last&&last.shadow){
        if(last.v!==28){
          ...
          txt='未触发入场：观察期内未回落至计划入场价'+last.entry+'，不计入胜率样本';
