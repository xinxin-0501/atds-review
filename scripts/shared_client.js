...
async function refreshWatchlistQuotes(){
  ...
  try{
    ...
    text.split(';').forEach(function(line){
      ...
      // v11.5:从 localStorage 读全量缓存(含 events/lhb/logic/minTrend/seal 等已补齐字段),仅覆盖实时行情字段。
      //        否则 5s 刷新会用精简对象重建决策卡,把 addFetchedToWatchlist 刚补齐的字段全部清空——这正是"删除重加后 detail 缺事件/龙虎榜/逻辑"的根因。
      ...
      if(d&&d.classList.contains('auto-detail')){
        ...
        var sObj=Object.assign({},fullData,{code:code,name:f[1],price:price,pct:pct,amount:amount,turnover:turnover,prevClose:prevClose,open:open,high:high,low:low,amplitude:amplitude,volRatio:volRatio,tech:tech||fullData.tech||null,fundFlow:ff||fullData.fundFlow||null});
