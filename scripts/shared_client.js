...
function buildDecisionCardHtml(s){
  ...
  var diMissing=[];
  if(ff.d1==null||isNaN(ff.d1))diMissing.push('资金');
...
async function fetchStockFundFlowF(code){
  ...
  try{
    ...
    if(kl.length>=3){
      ...
      // 写入浏览器本地缓存(供失败兜底)
      try{var fc=JSON.parse(localStorage.getItem('atds_ff_cache')||'{}'); fc[code]={...ff,date:new Date().toISOString().slice(0,10)}; localStorage.setItem('atds_ff_cache',JSON.stringify(fc));}catch(e2){}
      return ff;
    }
  }catch(e){/* 降级 */}
  ...
  try{
    ...
    try{var fc=JSON.parse(localStorage.getItem('atds_ff_cache')||'{}'); fc[code]={...ff2,date:new Date().toISOString().slice(0,10)}; localStorage.setItem('atds_ff_cache',JSON.stringify(fc));}catch(e2){}
    return ff2;
  }catch(e){/* 降级到缓存 */}
  // 失败兜底:浏览器本地缓存
  try{var fc=JSON.parse(localStorage.getItem('atds_ff_cache')||'{}'); if(fc[code]&&fc[code].d1!=null) return {d1:fc[code].d1,d3:fc[code].d3,d5:fc[code].d5};}catch(e){}
  return null;
}
