...
function restoreSavedWatchlist(){
  ...
  var card=document.querySelector('.watchlist-card');
  if(!card)return;
  // 1. 隐藏用户删除过的 config 股票(防刷新复活)
  hidden.forEach(function(c){var el=document.querySelector('.wl-stock[data-stock-code="'+c+'"]');if(el)el.remove();});
  // 2. 收集 config 已渲染 codes
  var configCodes=[];
  ...
  // 3. localStorage 中不在 config 的代码 → 拉最新行情追加
  var extraCodes=[];
  if(raw!==null){
  ...
  setTimeout(function(){try{refreshWatchlistQuotes();}catch(e){}}, 800);
}
