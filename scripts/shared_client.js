...
function resetLocalCache(){
  ...
  var CACHE_KEYS=['atds_watchlist_data','atds_watchlist_hidden','atds_kline_cache','atds_ff_cache','atds_events_cache','atds_trade_status','atds_shadow','atds_qr_code'];
...
async function fetchLhbDetailF(code){
  ...
  try{
    ...
    if(!rows.length){
      // 降级:读云端 lhb_cache.json
      return await tryCloudLhbCache(num,code);
    }
    ...
    // 东财主源异常时也走云端兜底
    try{return await tryCloudLhbCache(num,code);}catch(e2){return null;}
  }
}
