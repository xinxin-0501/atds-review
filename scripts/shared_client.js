    ...
    for(var i=0;i<anns.length;i++){
      ...
      for(var k=0;k<KW.length;k++){
        if(title.indexOf(KW[k][0])>=0){
          ...
          var left=daysLeft(d);
          evs.push({type:KW[k][1],eventType:KW[k][1],name:title.slice(0,30),title:title.slice(0,30),date:d,eventDate:d,left:left,countdown:left>0?('T-'+left+'天'):(left<0?Math.abs(left)+'天前':'今日'),dir:dir2,direction:dir2,level:KW[k][3],impactLevel:KW[k][3],detail:title.slice(0,48),source:'巨潮'});
          break;
        }
      }
    }
