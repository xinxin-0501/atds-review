...
function resetLocalCache(){
  // 缓存类(可清):data caches + 隐藏标记 + 交易状态 + 模拟跟踪 + 二维码
  var CACHE_KEYS=['atds_watchlist_data','atds_watchlist_hidden','atds_kline_cache','atds_ff_cache','atds_lhb_cache','atds_events_cache','atds_profile_cache','atds_trade_status','atds_shadow','atds_qr_code'];
  // 保留类:atds_watchlist(手动加股列表)、sim_track_*(其他模块的独立 checkbox 状态)
  // 额外清理 sim_track_* 单独的 checkbox 状态
  var simKeys=[];
  try{for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k&&k.indexOf('sim_track_')===0)simKeys.push(k);}}catch(e){}
  var removed=[];
  CACHE_KEYS.forEach(function(k){try{localStorage.removeItem(k);removed.push(k);}catch(e){}});
  // sim_track_* 也清掉(模拟跟踪与重置后的最新价对齐)
  simKeys.forEach(function(k){try{localStorage.removeItem(k);removed.push(k);}catch(e){}});
  // 读取 atds_watchlist 用于日志
  ...
  console.log('[resetLocalCache] 已清空 keys:', removed.join(', '), '· 保留 atds_watchlist(', watchlistCount, '只手动加股)');
  if(typeof showToast==='function'){showToast('已重置本地缓存('+removed.length+' 项),保留 '+watchlistCount+' 只手动加股,刷新页面强制从云端重新拉取','ok');}
...
async function fetchStockFundFlowF(code){
  ...
  function writeCache(ff,src,seriesIn){
    try{
      ...
      else if(ff.d1!=null&&!isNaN(ff.d1)&&!isWeekendKey(today))series[today]=ff.d1;
...
async function fetchEventsF(code){
  ...
  try{
    ...
    lc2[num]=events;
    lc2._meta=lc2._meta||{};
    lc2._meta[num]={date:new Date().toISOString().slice(0,10),src:srcUsed,count:events.length};
...
// v11.21:模拟跟踪结算。原实现只看"现价"是否 ≥止盈/≤止损,盘中冲高回落会被漏判;
//        改为用当日最高/最低判定触达,并支持"持有超期未触发"按现价结算,同时记录 lastCheck 供"跟踪中"展示。
function settleShadowTrade(code){
  ...
  var doSettle=function(result,pnl,note){
    rec.settled=true;rec.result=result;rec.pnl=pnl;rec.settleDate=today;rec.settleNote=note||'';
    addJournalEntry({code:code,name:rec.name||code,date:today,entry:entry,stop:stop,target:target,close:price,pnl:pnl,shadow:true,result:result,note:note||'',openedDate:rec.openedDate||'',strategy:'swing'});
  };
  var hitT=high>=target, hitS=low<=stop;
  if(hitT&&hitS){ doSettle('loss',pct(stop),'当日最高触止盈、最低触止损，保守计为止损'); }
  else if(hitT){ doSettle('win',pct(target),'当日最高触及止盈'); }
  else if(hitS){ doSettle('loss',pct(stop),'当日最低触及止损'); }
  else if(rec.openedDate){
    var held=Math.floor((new Date(today)-new Date(rec.openedDate))/86400000);
    if(held>=10&&price){ doSettle('expired',pct(price),'持有'+held+'日未触及止盈/止损，按现价结算'); }
  }
  setShadowMap(sm);
}
...
// v11.21:归因文本动态化。原实现每块固定显示"待人工复盘",用户无法确认系统是否真的在跟踪。
//        现按每只股票自己的模拟跟踪/真实交易记录生成状态;顶部进度/胜率按全部样本聚合(模拟与真实分列)。
function updateReviewProgress(){
  var j=getJournal();
  var sm=getShadowMap();
  var wins=0,losses=0,expired=0,sum=0,settled=0;
  for(var i=0;i<j.length;i++){
    var p=Number(j[i].pnl);
    var rs=j[i].result;
    if(!rs&&!isNaN(p)&&j[i].pnl!=null)rs=(p>=0?'win':'loss');   // v11.22:兼容旧版无 result 字段的历史记录,避免"进度有数、已结算为0"的不一致
    if(rs==='win'||rs==='loss'||rs==='expired'){
      settled++;
      if(!isNaN(p))sum+=p;
      if(rs==='win')wins++;else if(rs==='loss')losses++;else expired++;
    }
  }
  var avg=settled?Math.round(sum/settled*100)/100:0;
  var openCnt=0;
  for(var c in sm){ if(sm[c]&&sm[c].enabled&&!sm[c].settled)openCnt++; }
  ...
  for(var b=0;b<blocks.length;b++){
    ...
    if(r&&r.enabled&&!r.settled){
      ...
      var held=r.openedDate?Math.floor((Date.now()-new Date(r.openedDate))/86400000):0;   // v11.23:当天应为"已0天"(原 round 会显示已1天)
      txt='跟踪中（已'+held+'天）：现价'+px+'，距止盈'+(gap!=null?(gap>=0?'+':'')+gap+'%':'--')+'（止损'+r.stop+'），触达即自动结算';
    } else {
      var mine=[];
      for(var q=0;q<j.length;q++){ if(String(j[q].code)===String(code))mine.push(j[q]); }
      var last=mine.length?mine[mine.length-1]:null;
      if(last&&last.shadow){
        var rn={win:'止盈',loss:'止损',expired:'过期未触发'}[last.result]||(last.result||'已结算');
        txt='模拟已结算：'+rn+' '+(last.pnl>0?'+':'')+(last.pnl==null?'--':last.pnl)+'%'+(last.note?'（'+last.note+'）':'');
      } else if(last&&!last.shadow){
        txt='真实交易已记录：'+(last.pnl!=null?((last.pnl>0?'+':'')+last.pnl+'%'):'盈亏待补');
      } else {
  ...
  for(var k=0;k<els.length;k++)els[k].textContent=j.length+'/10';
  var wr=(wins+losses)?Math.round(wins/(wins+losses)*100):null;
  var line=settled
    ? (' · 已结算'+settled+'笔'+(wr!=null?('·胜率'+wr+'%'):'')+(expired?('·过期'+expired):'')+'，平均'+(avg>0?'+':'')+avg+'%'+((wins+losses+expired)<10?'（样本不足10笔，仅供参考）':''))
    : (' · 跟踪中'+openCnt+'笔（未结算）');
