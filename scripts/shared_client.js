  ...
  for(var attempt=0;attempt<3;attempt++){
    try{
      ...
      if(kl.length>=3){
        ...
        return writeCache(ff,'eastmoney-push2his',seriesIn)||ff;
      }
    }catch(e){/* 重试 */}
  }
  // 降级1.5:新浪资金流历史(v11.18)。东财历史源 push2his 在部分网络不可达(实测 Actions 侧间歇不可达),
  //        手动新增个股又无云端缓存兜底 → 用新浪逐日 r0_net(主力=大单+超大单净额)补齐 d1/d3/d5。
  //        口径与东财 f52 不完全一致,故 source 标 'sina-r0',渲染层显示「新浪口径」徽章,不伪装成东财数据。
  try{
    var sinaUrl='https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/MoneyFlow.ssl_qsfx_zjlrqs?page=1&num=10&sort=opendate&asc=0&daima='+(mkt==='1'?'sh':'sz')+num;
    var resS=await fetch(sinaUrl,{headers:{'User-Agent':'Mozilla/5.0','Referer':'https://finance.sina.com.cn/'}});
    var jsS=await resS.json();
    if(Array.isArray(jsS)&&jsS.length){
      var mapS={};
      for(var si2=0;si2<jsS.length;si2++){
        var rowS=jsS[si2];if(!rowS||!rowS.opendate)continue;
        var vS=parseFloat(rowS.r0_net);if(isNaN(vS))continue;
        mapS[String(rowS.opendate)]=Math.round(vS/1e6)/100;
      }
      var daysS=Object.keys(mapS).sort();
      if(daysS.length){
        var sumS=function(k){var arr=daysS.slice(-k),t=0;for(var q=0;q<arr.length;q++)t+=mapS[arr[q]];return Math.round(t*100)/100;};
        var ffS={d1:mapS[daysS[daysS.length-1]],d3:daysS.length>=3?sumS(3):null,d5:daysS.length>=5?sumS(5):null};
        var seriesInS={};for(var q2=0;q2<daysS.length;q2++)seriesInS[daysS[q2]]=mapS[daysS[q2]];
        var recS=writeCache(ffS,'sina-r0',seriesInS);
        return recS||ffS;
      }
    }
  }catch(e){/* 新浪失败,继续降级 */}
  // 降级1:实时接口(仅当日),d3/d5 置 null 避免重复
  try{
    ...
    if(kl2.length){
      ...
      return writeCache(ff2,'eastmoney-push2',seriesIn2)||ff2;
    }
  }catch(e){/* 降级2:云端 fflow_cache.json */}
