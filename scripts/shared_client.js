...
async function fetchEventsF(code){
  ...
  // 巨潮公告(减持/增发/回购/股东大会/问询):客户端直连 cninfo(国内手机 IP),失败时降级到缓存
  ...
  }catch(e){/* cninfo 客户端失败,继续读缓存 */}
  // 缓存兜底:如果所有源都失败(events 空)→ 读云端 events_cache.json + localStorage
  if(!events.length){
    try{
