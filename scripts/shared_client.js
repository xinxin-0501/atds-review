...
async function addFetchedToWatchlist(code){
  ...
  try {
    ...
    // 分钟线兜底:双源失败(如浏览器CORS拦截腾讯/新浪)时用日线MA趋势兜底,坚决不显示"--"
    var f60 = ext[0], f15 = ext[1];
    if (!f60 && data.tech && data.tech.trend) f60 = { trend: data.tech.trend, ma5: data.tech.ma5, ma10: data.tech.ma10, fallback: true };
    if (!f15 && data.tech && data.tech.trend) f15 = { trend: data.tech.trend, ma5: data.tech.ma5, ma10: data.tech.ma10, fallback: true };
...
function buildDecisionCardHtml(s){
  ...
  function minLabel(m){if(!m)return '数据暂缺';var dir=m.trend==='up'?'多头':m.trend==='down'?'空头':'震荡';var ma=m.ma10!=null?('MA10 '+m.ma10):(m.ma5!=null?('MA5 '+m.ma5):'');var fb=m.cached?'·缓存':(m.fallback?'·日线兜底':'');return ma?(dir+'('+ma+')'+fb):(dir+fb);}
  var minTxt='60分 '+minLabel(m60)+' · 15分 '+minLabel(m15);
  ...
  var tStop=price-Math.max(0.5*atrC,price*0.01);
  var tTgt=price+Math.max(0.5*atrC,price*0.01);
  var tRR=(price>tStop)?((tTgt-price)/(price-tStop)):1;
  var tRRBad=tRR<1.5;
  // 状态标签降级:方案C盈亏比不合格(做T风险极高)且主力资金流出 → 禁止"轻仓试错",强制"高风险观察"
  var riskDowngrade=tRRBad&&(ff.d1||0)<0;
  var effStatusLabel=riskDowngrade?'高风险观察':statusLabel;
  var effConfTone=riskDowngrade?'down':confTone;
  ...
  var planTable='<table class="tp-table"><tr><th>方案</th><th>触发条件</th><th>入场</th><th>止损</th><th>止盈</th><th>盈亏比</th></tr>'+
    ...
    planRow('C 日内做T','现价'+f2(price)+'·ATR'+f2(atrC)+'动态止损',price,tStop,tTgt,tRR,trigC,rrTone(tRR),tRRBad)+
    '</table>';
  ...
  var patternHtml=upperShadow?'<div class="dc-line dc-pattern">形态判定：今日大幅冲高回落，收长上影线，上方抛压极重，空头占优。</div>':'';

  // 情绪总纲建议:情绪冰点+趋势空头+主力流出 → 高难度逆势标的
