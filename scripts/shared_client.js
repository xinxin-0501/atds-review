...
function buildDecisionCardHtml(s){
  ...
  return '<div class="dc-head">'+
    ...
    '<div class="dc-block dc-review" data-review-code="'+escHtmlF(s.code)+'" data-name="'+escHtmlF(s.name)+'" data-price="'+price+'" data-high="'+(Number(s.high)||price)+'" data-low="'+(Number(s.low)||price)+'" data-entry="'+Number(p.entry).toFixed(2)+'" data-stop="'+Number(p.stop).toFixed(2)+'" data-target="'+Number(p.target).toFixed(2)+'"><div class="dc-h">📊 盘后复盘（当日验证）</div>'+
      ...
      '<div class="dc-line">策略执行/归因：<span class="ts-attr">未激活 —— 勾选"系统模拟跟踪"后开始累计样本</span> <span class="ts-hint" title="两阶段结算：① 需当日最低回落至计划入场价才算入场（观察起点若已跌破计划止损位则判为计划失效，不计入；观察满10日未触及则判为未触发入场）② 入场当日不判止损 —— 当日最低可能出现在入场之前(实测:当日早盘探底后拉升),故入场当日改用 15 分钟K判定「入场之后」的路径,取不到分钟K则当日不判定、改为次日起按日K判定;次日起用当日最高/最低判定止盈/止损并自动结算，同日双触保守计为止损。每笔的入场/结算日期与价格见顶部「每笔明细」与口径说明；有效样本≥10 笔才展示胜率">?</span> <span class="ts-progress-badge">进度 <b class="ts-progress">0/10</b></span> <span class="ts-winrate"></span></div>'+
...
//   ② 已入场:入场次日起用当日最高/最低判定;同日双触保守计为止损;入场满 10 日未触达 → expired(按现价)
...
function settleShadowTrade(code,klines,minBars){
  ...
  // ③ 超期出口
  if(!rec.entryHit){
    var wd=bjDaysBetweenF(rec.openedDate,today);
    if(wd>=10)doSettle('missed',0,null,'观察'+wd+'日未回落至计划入场价'+entry+'，未触发入场');
  } else {
    var held=bjDaysBetweenF(rec.entryDate||rec.openedDate,today);
    if(held>=10)doSettle('expired',pct(price),price,'入场后持有'+held+'日未触及止盈/止损，按现价结算');
  }
  setShadowMap(sm);
}
