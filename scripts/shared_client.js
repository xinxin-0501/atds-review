...
function buildDecisionCardHtml(s){
  ...
  return '<div class="dc-head">'+
    ...
    '</div>'+
    '<div class="dc-block dc-review" data-review-code="'+escHtmlF(s.code)+'" data-name="'+escHtmlF(s.name)+'" data-price="'+price+'" data-high="'+(Number(s.high)||price)+'" data-low="'+(Number(s.low)||price)+'" data-entry="'+p.entry+'" data-stop="'+p.stop+'" data-target="'+p.target+'"><div class="dc-h">📊 盘后复盘（当日验证）</div>'+
...
function toggleShadowTrack(cb){
  ...
  var m=getShadowMap();
  if(cb.checked&&block){
    m[code]={enabled:true,name:block.getAttribute('data-name')||code,entry:parseFloat(block.getAttribute('data-entry'))||null,stop:parseFloat(block.getAttribute('data-stop'))||null,target:parseFloat(block.getAttribute('data-target'))||null,openedDate:bjTodayF(),settled:false};
