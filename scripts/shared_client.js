...
function buildDecisionCardHtml(s){
  ...
  function minLabel(m){if(!m)return '--';var dir=m.trend==='up'?'多头':m.trend==='down'?'空头':'震荡';var ma=m.ma10!=null?('MA10 '+m.ma10):(m.ma5!=null?('MA5 '+m.ma5):'');return ma?(dir+'('+ma+')'):dir;}
  ...
  function planRow(name,trig,entryV,stopV,tgtV,rrVal,triggered,tone){return '<tr class="'+(triggered?'':'tp-notrig')+'"><td class="tp-name">'+name+'</td><td class="tp-trig">'+escHtmlF(trig)+'</td><td class="tp-num">'+f2(entryV)+'</td><td class="tp-num stop">'+f2(stopV)+'</td><td class="tp-num">'+f2(tgtV)+'</td><td class="tp-rr '+(triggered?'rr-'+tone:'tp-rr-muted')+'">'+(triggered?rrVal.toFixed(2):'未触发')+'</td></tr>';}
  var planTable='<table class="tp-table"><tr><th>方案</th><th>触发条件</th><th>入场</th><th>止损</th><th>止盈</th><th>盈亏比</th></tr>'+
    ...
    planRow('C 日内做T','现价'+f2(price)+'·ATR'+f2(atrC)+'动态止损',price,tStop,tTgt,tRR,trigC,rrTone(tRR))+
    '</table>';
  ...
  var summaryHtml=(emotionCycle==='冰点'&&t.trend==='down'&&(ff.d1||0)<0)?'<div class="dc-summary">⛔ 情绪冰点+趋势空头+主力流出，属于高难度逆势标的，系统强烈建议不参与，仅作观察。</div>':'';
