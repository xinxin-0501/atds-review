...
function toggleShadowTrack(cb){
  ...
  if(cb.checked&&block){
    ...
    m[code]={enabled:true,name:block.getAttribute('data-name')||code,entry:_en,stop:r2fF(block.getAttribute('data-stop')),target:r2fF(block.getAttribute('data-target')),openedDate:bjTodayF(),settled:false,entryType:_et,refPrice:_px};
  ...
  for(var j=0;j<seq.length;j++){
    ...
    if(!entered){
      if(s.lo<=entry){
        ...
        if(s.lo<=stop){
          trace.push({d:s.d,hi:s.hi,lo:s.lo,ev:'enter-stop'});
          out={result:'loss',date:s.d,price:stop,note:'入场当日即跌破止损（日K回补）'};
          break;
        }
...
function settleShadowTrade(code,klines){
  var sm=getShadowMap();
  var rec=sm[code];
  ...
  if(!kUsed){
    ...
    if(ev==='enter-stop'){ doSettle('loss',pct(stop),stop,'入场当日即跌破止损'); return; }
...
async function settleAllShadowTrades(){
  ...
  for(var j=0;j<codes.length;j++){
    try{ settleShadowTrade(codes[j],kls[codes[j]]); }catch(e){ console.warn('[settleShadowTrade]',codes[j],e.message); }
  }
  updateReviewProgress();
}
...
function renderShadowTrace(blockEl,code,rec){
  ...
  if(traceOpen){
    ...
    for(var i=tr.length-1;i>=0;i--){
      ...
      else if(x.ev==='hold')note+='（当日最高 '+x.hi+'，距止盈 '+((x.gapT!=null&&x.gapT>=0)?(x.gapT+'%'):'已越过')+'）';
