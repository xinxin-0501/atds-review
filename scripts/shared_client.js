  ...
  try{
    ...
    text.split(';').forEach(function(line){
      ...
      if(d){
        ...
        // 2. 重新计算 detail-grid 内"风险/风控/建议"文本(基于最新 pct/turnover/atds)
        var turnNum=Number(turnover);
        var riskL=deriveRiskLevelF(pct);
        var horizonL=deriveTimeHorizonF(pct,turnNum);
        var adviceL=deriveAdviceF(pct,atds,riskL.tone);
        var riskLines=deriveRiskTextF(pct,turnNum);
        var horizons=deriveHorizonLinesF(pct,turnNum);
        var adviceText=deriveAdviceTextF(pct,atds,riskL.tone);
        var blocks=d.querySelectorAll('.detail-block');
        if(blocks.length>=3){
          // 风险块(第 1 个):更新 riskName + 列表
          var riskNameEl=blocks[0].querySelector('.risk-tag');
          if(riskNameEl){riskNameEl.className='risk-tag risk-'+riskL.tone;riskNameEl.textContent=riskL.name;}
          var riskListEl=blocks[0].querySelectorAll('.detail-line');
          riskListEl.forEach(function(el,idx){if(riskLines[idx]!=null)el.textContent=riskLines[idx];});
          // 风控块(第 2 个):更新 horizonName + 3 行短线/波段/长线
          var horizonNameEl=blocks[1].querySelector('.horizon-tag');
          if(horizonNameEl){horizonNameEl.className='horizon-tag horizon-'+horizonL.tone;horizonNameEl.textContent=horizonL.name;}
          var horizonRows=blocks[1].querySelectorAll('.detail-line');
          horizonRows.forEach(function(el,idx){
            if(horizons[idx]){
              var b=el.querySelector('b');if(b)b.textContent=horizons[idx].k;
              // 文本节点(去除 b 后剩余部分)
              var txtNode=Array.from(el.childNodes).find(function(n){return n.nodeType===3&&n.textContent.trim()!==''});
              if(txtNode)txtNode.textContent=horizons[idx].v;
              else if(!b){el.textContent=horizons[idx].k+horizons[idx].v;}
            }
          });
          // 建议块(第 3 个):更新 adviceName + text
          var adviceNameEl=blocks[2].querySelector('.advice-tag');
          if(adviceNameEl){adviceNameEl.className='advice-tag advice-'+adviceL.tone;adviceNameEl.textContent=adviceL.name;}
          var adviceTextEl=blocks[2].querySelector('.detail-line');
          if(adviceTextEl){adviceTextEl.textContent=adviceText;}
        }
      }
    });
