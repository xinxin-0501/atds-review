...
function buildDecisionCardHtml(s){
  ...
  var evHtml=(evs&&evs.length)
    ?evs.slice(0,10).map(function(e){var red=(e.level==='高'&&e.left>=-3&&e.left<=3)?' ev-red-alert':'';var cnt=e.left>0?(' T-'+e.left+'天'):(e.left<0?(' '+Math.abs(e.left)+'天前'):' 今日');var rawTitle=e.fullTitle||e.name||'';var showTitle=rawTitle?(rawTitle.length>28?(rawTitle.slice(0,28)+'…'):rawTitle):'';var linkHtml=e.cninfoUrl?'<a class="ev-link" href="'+escHtmlF(e.cninfoUrl)+'" target="_blank" rel="noopener" title="查看公告原文">查看原文↗</a>':'';return '<span class="ev-item ev-'+(e.level==='高'?'h':e.level==='中'?'m':'l')+' '+(e.dir==='利好'?'ev-good':e.dir==='利空'?'ev-bad':'')+red+'"><span class="ev-head"><span class="ev-type">'+escHtmlF(e.type)+'</span>'+(e.dir&&e.dir!=='中性'?'<span class="ev-dir ev-dir-'+(e.dir==='利好'?'good':'bad')+'">'+escHtmlF(e.dir)+'</span>':'')+'<span class="ev-cnt">'+cnt+'</span>'+(e.source?'<span class="ev-src">〔'+escHtmlF(e.source)+'〕</span>':'')+'</span><span class="ev-title" title="'+escHtmlF(e.detail||rawTitle||e.date||'')+'">'+escHtmlF(showTitle)+'</span>'+linkHtml+'</span>';}).join('')
    :evEmpty;
...
async function fetchEventsF(code){
  ...
  events.sort(function(a,b){return (lvRank[a.level]!=null?lvRank[a.level]:2)-(lvRank[b.level]!=null?lvRank[b.level]:2)||(a.date<b.date?-1:a.date>b.date?1:0);});
