...
function settleShadowTrade(code){
  ...
  if(result){
    ...
    setShadowMap(sm);
    addJournalEntry({code:code,name:rec.name||code,date:new Date().toISOString().slice(0,10),entry:entry,stop:stop,target:target,close:price,pnl:pnl,shadow:true,result:result});
  }
}
