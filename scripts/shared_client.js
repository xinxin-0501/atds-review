...
function buildDecisionCardHtml(s){
  ...
  return '<div class="dc-head">'+
    ...
    '<div class="dc-block dc-review" data-review-code="'+escHtmlF(s.code)+'" data-name="'+escHtmlF(s.name)+'" data-price="'+price+'" data-high="'+(Number(s.high)||price)+'" data-low="'+(Number(s.low)||price)+'" data-entry="'+p.entry+'" data-stop="'+p.stop+'" data-target="'+p.target+'"><div class="dc-h">📊 盘后复盘（当日验证）</div>'+
      ...
      '<div class="dc-line">策略执行/归因：<span class="ts-attr">尚未跟踪 —— 勾选"系统模拟跟踪"后开始累计样本</span> <span class="ts-hint" title="两阶段结算：① 需当日最低回落至计划入场价才算入场（观察起点若已跌破计划止损位则判为计划失效，不计入；观察满10日未触及则判为未触发入场）② 入场次日起用当日最高/最低判定止盈/止损并自动结算，同日双触保守计为止损。每笔的入场/结算日期与价格见顶部「每笔明细」与口径说明；有效样本≥10 笔才展示胜率">?</span> <span class="ts-progress-badge">进度 <b class="ts-progress">0/10</b></span> <span class="ts-winrate"></span></div>'+
...
function settleShadowTrade(code){
  ...
  // v11.29 防御1:计划参数必须合法(止盈>入场>止损)。非法计划一律不参与判定,避免脏计划产出结论。
  if(!entry||!stop||!target||!(target>entry&&entry>stop))return;
  // v11.29 防御2:价格数据必须为正且区间自洽。
  if(!(price>0&&high>0&&low>0&&high>=low))return;
  var today=bjTodayF();
  // v11.29 防御3:振幅异常(>50%)视为数据异常或疑似除权(主板最大振幅约20%、双创约40%)→ 本次不判定,只记录。
  if((high-low)/low>0.5){
  ...
  var doSettle=function(result,pnl,settlePrice,note){
    rec.settled=true;rec.result=result;rec.pnl=pnl;rec.settleDate=today;rec.settlePrice=settlePrice;rec.settleNote=note||'';
    addJournalEntry({code:code,name:rec.name||code,date:today,entry:entry,stop:stop,target:target,close:price,pnl:pnl,shadow:true,result:result,note:note||'',
      openedDate:rec.openedDate||'',entryDate:rec.entryDate||'',settleDate:today,settlePrice:settlePrice,v:28,strategy:'swing'});
    setShadowMap(sm);
  };
  // v11.29 防御4:观察起点作废。首个有效检查日若现价已在【计划止损位之下】,说明勾选时计划已被击穿,
  //   现实中不会按该计划入场(否则等于"明知破位还照价买")。此类记为 invalid(计划失效),不计入胜率。
  //   注意:只筛"起点已破位"这一情形,不影响"入场后正常止损"的亏损样本(那类必须保留,否则又会系统性虚高胜率)。
  ...
  // 阶段2:已入场(入场当日只判止损,不判止盈)
  if(rec.entryDate===today){
    if(low<=stop)doSettle('loss',pct(stop),stop,'入场当日即跌破止损');
    setShadowMap(sm);
    return;
  }
  var hitT=high>=target, hitS=low<=stop;
  if(hitT&&hitS){ doSettle('loss',pct(stop),stop,'当日最高触止盈、最低触止损，保守计为止损'); }
  else if(hitT){ doSettle('win',pct(target),target,'入场后当日最高触及止盈'); }
  else if(hitS){ doSettle('loss',pct(stop),stop,'当日最低触及止损'); }
  else if(rec.openedDate){
    var held=bjDaysBetweenF(rec.entryDate||rec.openedDate,today);
    if(held>=10&&price){ doSettle('expired',pct(price),price,'入场后持有'+held+'日未触及止盈/止损，按现价结算'); }
  }
  setShadowMap(sm);
}
